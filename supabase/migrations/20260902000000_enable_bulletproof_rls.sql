-- =========================================================================
-- AKORNO PRODUCTION ROW LEVEL SECURITY (RLS) & SCHEMA DEFINITIONS
-- =========================================================================
...
CREATE POLICY "Users can view referrals where they are inviter or referred"
  ...
  TO authenticated
  USING (auth.uid() = inviter_id OR auth.uid() = referred_user_id);
...
CREATE POLICY "Authenticated users can record referral join"
  ...
  TO authenticated
  WITH CHECK (auth.uid() = referred_user_id OR referred_user_id IS NULL);
...
BEGIN
  ...
  INSERT INTO public.user_gamification (user_id, grace_xp, streak_days, level)
  VALUES (NEW.id, 50, 1, 1)
